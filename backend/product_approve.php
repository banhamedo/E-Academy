...
try {
  ...
  $pdo->exec("CREATE TABLE IF NOT EXISTS products LIKE products"); // no-op safeguard

  $stmt = $pdo->prepare("UPDATE products SET approval_status = 'approved', is_active = 1 WHERE id = ?");
  $stmt->execute([$productId]);

  echo json_encode(['success' => true, 'product_id' => $productId, 'status' => 'approved']);
} catch (Throwable $e) {
