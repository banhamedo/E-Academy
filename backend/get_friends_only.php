...
try {
    ...
    // First, let's check if friends table exists and has data
    $checkFriends = $pdo->query("SELECT COUNT(*) as count FROM friends");
    $friendsCount = $checkFriends->fetch(PDO::FETCH_ASSOC)['count'];
    
    if ($friendsCount == 0) {
        // No friends data, return empty array with message
        echo json_encode([
            'success' => true, 
            'users' => [], 
            'message' => 'لا توجد صداقات مضافة بعد',
            'debug' => ['friends_count' => $friendsCount]
        ]);
        exit;
    }
    ...
    $formattedUsers = array_map(function($user) {
        return [
            ...
            'userType' => $user['user_type'] ?: 'صديق',
            'joinedDate' => $user['created_at'],
            'friendStatus' => $user['friend_status'] ?: 'friends',
